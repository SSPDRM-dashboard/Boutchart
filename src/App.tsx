...
const DEMO_DATA = `Name,Club,Category,School,Gender,Description
...
Raj Kumar,Phoenix Club,-60kg,SIT,Male,Varsity Qualifier
Hafiz Rahman,Lotus Gym,-60kg,SUSS,Male,Open Division
Marcus Lee,Star Gym,-66kg,NUS,Male,Seed 1
Kavin Selvam,Dragon Club,-66kg,NTU,Male,Regional Medalist
Yusuf Ibrahim,Tiger Gym,-66kg,SMU,Male,Varsity Contender
Daniel Wong,Eagle Judo Club,-66kg,SIT,Male,Junior Gold Medalist
Amir Hakim,Phoenix Club,-73kg,SMU,Male,Senior Black Belt
Brandon Goh,Star Gym,-73kg,NUS,Male,Top Contender
Faiz Rosli,Lotus Gym,-73kg,NTU,Male,Open Weight Finalist
...
Jane Doe,Tiger Gym,-57kg,NUS,Female,Seed 1 / Inter-Club Champion
Clara Oswald,Phoenix Club,-57kg,SIT,Female,Varsity Contender`;
...
export default function App() {
          ...
          decompressFromGzipBase64(dataParam)
            ...
            .then(parsed => {
              if (parsed) {
                ...
                if (parsed.leftLogo2) setLeftLogo2(parsed.leftLogo2);
                if (parsed.rightLogo) setRightLogo(parsed.rightLogo);
                if (parsed.rightLogo2) setRightLogo2(parsed.rightLogo2);
  ...
  const handleLoadRoster = (text: string, source: string, adminNotes?: string) => {
    if (!text.trim()) {
      setStatusMessage({ text: 'Please select a CSV file or paste spreadsheet rows to continue.', type: 'err' });
      return;
    }

    try {
      const parsed = buildRosterFromText(text, adminNotes);
      if (parsed.length === 0) {
  ...
  const handleLoadSavedEvent = (id: string) => {
    ...
    const performLoad = () => {
      ...
      setLeftLogo2(target.leftLogo2 || '');
      setRightLogo(target.rightLogo || '');
      setRightLogo2(target.rightLogo2 || '');
  ...
  const handleDownloadSearchablePdf = async (ringFilter?: 'all' | number) => {
    ...
    try {
      ...
      for (let index = 0; index < keysToExport.length; index++) {
        ...
        // Draw Logos if they exist (using preloaded HTMLImageElement for robust format support)
        let leftOffsetX = 20;
        if (loadedLeftLogo) {
          try {
        ...
        for (let k = 0; k <= numRounds; k++) {
          ...
          for (let i = 0; i < count; i++) {
            let x: number;
            if (k === numRounds) {
              x = PAD + (numRounds - 1) * gap + BOX_W / 2 + centerOffset;
            } else {
              const isLeft = i < count / 2;
              if (isLeft) {
                x = PAD + k * gap;
              } else {
                x = PAD + (2 * numRounds - 2 - k) * gap + BOX_W + centralGap;
              }
            }
        ...
        const canvasWidth = numRounds === 0 ? PAD * 2 + BOX_W : PAD * 2 + (2 * numRounds - 2) * gap + 2 * BOX_W + centralGap;
                        ...
                        <div>
                          ...
                          <div className="flex flex-wrap items-center gap-3 mt-4">
                            <button
                              ...
                              className="px-5 py-2.5 bg-amber-500 hover:bg-amber-400 text-slate-950 rounded-xl text-xs font-black shadow transition-all cursor-pointer flex items-center gap-2"
                            >
                              <span>🥋</span>
                              <span>Go to Draws & Brackets</span>
                            </button>
                            <button
                              onClick={handleLogout}
                              className="px-5 py-2.5 bg-slate-900 border border-slate-800 text-white rounded-xl text-xs font-bold shadow hover:bg-slate-800 transition-colors cursor-pointer"
                            >
                              Sign Out
                            </button>
                          </div>
        ...
        <div className="fixed inset-0 z-[200] overflow-y-auto no-print">
          ...
          <div className="flex min-h-full items-center justify-center p-4 text-center animate-fade-in">
            <div className="relative transform overflow-hidden rounded-2xl bg-white text-left shadow-2xl transition-all sm:my-8 w-full max-w-md border border-slate-100 p-6 space-y-5">
              ...
              {/* Quick Preview of Parsed Athletes Columns */}
              {pendingRosterImport.parsed.length > 0 && (
                <div className="bg-slate-50 border border-slate-200 rounded-xl p-3 text-left">
                  <div className="flex items-center justify-between text-[11px] font-bold text-slate-700 pb-1.5 mb-1.5 border-b border-slate-200">
                    <span>Parsed Columns Sample</span>
                    <span className="text-slate-400 font-normal">First {Math.min(3, pendingRosterImport.parsed.length)} rows</span>
                  </div>
                  <div className="space-y-1 max-h-28 overflow-y-auto">
                    {pendingRosterImport.parsed.slice(0, 3).map((ath, idx) => (
                      <div key={idx} className="text-[11px] font-mono text-slate-700 bg-white border border-slate-150 rounded px-2 py-1 flex items-center justify-between gap-2">
                        <span className="font-bold text-slate-900 truncate">{ath.name}</span>
                        <span className="text-amber-700 truncate">{ath.weight}</span>
                        <span className="text-slate-500 truncate text-[10px]">{ath.club || 'No Club'}</span>
                        {ath.description && (
                          <span className="text-emerald-700 truncate text-[10px] italic">({ath.description})</span>
                        )}
                      </div>
                    ))}
                  </div>
